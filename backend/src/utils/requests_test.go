...
func TestGetTagSuccessful(t *testing.T) {
	db, err := startingServer()
	require.Equal(t, nil, err, "Error in DB connection: %v", err)

	resp, err := http.Get("http://" + HOST + ":9080/tags/1")
	...
	item := &adapter.Tag{}
	err = db.Connection.Get(item, fmt.Sprintf("SELECT * FROM online_shop.tags WHERE id=1"))
	JSON, err := json.Marshal(item)
	expected := fmt.Sprintf("{\"tag\" : %v}", string(JSON))
	require.Equal(t, expected, bodyString)
}

func TestGetTagUnSuccessful(t *testing.T) {
	db, err := startingServer()
	require.Equal(t, nil, err, "Error in DB connection: %v", err)

	resp, err := http.Get("http://" + HOST + ":9080/tags/-5")

	require.Equal(t, nil, err, "Error in request: %v", err)
	require.Equal(t, http.StatusInternalServerError, resp.StatusCode, "Error in request: %v", http.StatusOK)
	require.Equal(t, "text/plain; charset=utf-8", resp.Header.Get("Content-Type"), "Error in header Content-Type: %v",
		err)

	bodyString, err := ReadBody(resp)

	item := &adapter.Tag{}
	err = db.Connection.Get(item, fmt.Sprintf("SELECT * FROM online_shop.tags WHERE id=-5"))
	expected := fmt.Sprintf("{\"status\":\"bad tag id\"}\n")
	require.Equal(t, expected, bodyString)
}

func TestGetTagsSuccessful(t *testing.T) {
	db, err := startingServer()
	require.Equal(t, nil, err, "Error in DB connection: %v", err)

	resp, err := http.Get("http://" + HOST + ":9080/tags/")
	...
	var response adapter.Tag
	err = json.Unmarshal([]byte(bodyString), &response)

	item := &[]adapter.Tag{}
	err = db.Connection.Select(item, fmt.Sprintf("SELECT * FROM online_shop.tags"))
	JSON, err := json.Marshal(item)
	expected := fmt.Sprintf("{\"tags\" : %v}", string(JSON))
	require.Equal(t, expected, bodyString)
}
