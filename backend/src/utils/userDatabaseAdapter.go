...
func (adapter *UserDatabase) createUser(user User) (id int64, err error) {
	res := adapter.database.Connection.MustExec("INSERT INTO online_shop.users (user_name, user_surname,user_patronymic, phone, birthdate, password_hash, mail, role_id) VALUES ($1, $2, $3, $4, $5, $6, $7, #8)", user.Name, user.Surname, user.Patronymic, user.Phone, user.Birthdate, user.Password, user.Mail, user.RoleId)
